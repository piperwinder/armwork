...
public class shoulderUpCommand extends CommandBase {
  ...
  private final XboxController shoulderStick;
   ...
   */
  public shoulderUpCommand(shoulderSubsystem shoulderSub, XboxController shoulderStick) {
    this.shoulderSub = shoulderSub;
    this.shoulderStick = shoulderStick;
  ...
  @Override
  public void execute() {
    double speed = -shoulderStick.getLeftY() * .5;
        if (speed < 0) {
            speed *= -speed;
        } else {
            speed *= speed;
